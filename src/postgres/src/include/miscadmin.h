...
/*
 * YB: We track critical section entry per-process so the postmaster can detect
 * when a child dies inside a critical section.  Like yb_spin_locks_acquired_ptr
 * in spin.h, we use an external pointer (set in proc.c when MyProc is
 * initialized) to avoid including proc.h here.
 */
#ifndef FRONTEND
extern PGDLLIMPORT bool *yb_entered_critical_section_ptr;
#endif
...
do { \
	if (yb_entered_critical_section_ptr) \
		*yb_entered_critical_section_ptr = true; \
	CritSectionCount++; \
} while(0)
...
do { \
	Assert(CritSectionCount > 0); \
	CritSectionCount--; \
	if (yb_entered_critical_section_ptr && CritSectionCount == 0) \
		*yb_entered_critical_section_ptr = false; \
} while(0)
#endif							/* YB: FRONTEND */
