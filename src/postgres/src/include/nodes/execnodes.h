...
/*
 * YbExprColrefDesc has been moved to primnodes.h so that
 * gen_node_support.pl auto-generates copy/equal/out/read functions for it.
 * (execnodes.h is in @nodetag_only_files, which only assigns node tags.)
 */
...
typedef struct TupleHashTableData
{
	...
	ExprState **yb_in_keycolExprs;	/* equivalent of yb_keyColExprs for input
									 * tuples */
	FmgrInfo   *yb_tab_hash_funcs;	/* hash functions for table datatype(s) */
	FmgrInfo   *yb_in_hash_funcs;	/* hash functions for input datatype(s) */
	uint32		yb_hash_iv;		/* hash-seed input for hash functions */
} TupleHashTableData;
