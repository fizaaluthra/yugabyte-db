...
/*
 * YbExprColrefDesc — DocDB column-reference descriptor for the YB single-row
 * modify optimization.  The planner replaces Var nodes with these when pushing
 * expressions down to DocDB.
 *
 * This struct lives in primnodes.h (not execnodes.h) so that
 * gen_node_support.pl auto-generates copy/equal/out/read functions for it.
 * execnodes.h is in @nodetag_only_files and only produces node tags.
 */
