...
setup_config(void)
{
	...
	/* YB: writefile() already frees conflines in PG19, no separate free needed */
	...
	if (!IsYugaByteGlobalClusterInitdb() && !IsYugaByteLocalNodeInitdb())
	{
		...
		*/
		...
		/* YB: writefile() already frees conflines in PG19 */
	}
...
make_yugabyte(FILE *cmdfd)
{
	...
	/*
	 * PG19's replace_token() frees the original strings, so we must use
	 * pg_strdup'd copies rather than string literals.
	 */
	lines = (char **) pg_malloc(4 * sizeof(char *));
	lines[0] = pg_strdup("CREATE USER yugabyte SUPERUSER INHERIT CREATEROLE CREATEDB LOGIN REPLICATION BYPASSRLS PASSWORD $YB_PASSWORD;\n\n");
	lines[1] = pg_strdup("CREATE DATABASE yugabyte $YB_OID;\n\n");
	lines[2] = pg_strdup("COMMENT ON DATABASE yugabyte IS 'default administrative connection database';\n\n");
	lines[3] = NULL;
...
initialize_data_directory(void)
{
	...
	/*
	 * YB: Skip system_constraints.sql in YB mode. PG19 generates
	 * ALTER TABLE ... ADD PRIMARY KEY/UNIQUE USING INDEX commands here,
	 * but YB already creates primary keys during table creation in the
	 * bootstrap phase. These ALTER TABLE commands would trigger expensive
	 * table rewrites in YB.
	 */
	if (!IsYugaByteGlobalClusterInitdb())
