		...
		te->dropStmt && strlen(te->dropStmt) > 0)
	{
		...
		else
		{
			...
			if (temp.data[0])
			{
				if (AH->public.dopt->include_yb_metadata)
				{
					PQExpBuffer alter_buf = createPQExpBuffer();

					appendPQExpBuffer(alter_buf, "ALTER %s OWNER TO %s;",
					...
					if (AH->public.dopt->yb_dump_role_checks)
					{
						...
						ahprintf(AH, "SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname = %s"
								 ...
								 "\\endif\n", role_buf->data, alter_buf->data, fmtId(te->owner));
