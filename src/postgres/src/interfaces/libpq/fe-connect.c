...
pqConnectOptions2(PGconn *conn)
{
	...
	for (i = 0; i < conn->nconnhost; i++)
	{
		...
		else
		{
			...
			/*
			 * YugaByte: default to TCP localhost instead of Unix socket.
			 *
			 * In PG15 we used a #undef HAVE_UNIX_SOCKETS trick to skip the
			 * Unix socket branch, but PG19 removed the #ifdef guard so that
			 * no longer works.  Directly select localhost for YB builds.
			 */
