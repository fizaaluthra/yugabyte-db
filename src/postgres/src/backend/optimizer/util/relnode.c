...
build_simple_rel(PlannerInfo *root, int relid, RelOptInfo *parent)
{
	...
	if (IsYugaByteEnabled())
	{
		char	   *rteHintAlias = NULL;
		uint32		rteUniqueBaseId = 0;

		if (rte->perminfoindex > 0)
		{
			RTEPermissionInfo *perminfo = getRTEPermissionInfo(root->parse->rteperminfos, rte);

			rteHintAlias = perminfo->ybHintAlias;
			rteUniqueBaseId = perminfo->ybUniqueBaseId;
		}

		if (rteHintAlias != NULL)
		{
			/*
			 ...
			 */
			rel->ybHintAlias = rteHintAlias;
			rel->ybBlockId = root->ybBlockId;
			rel->ybUniqueBaseId = rteUniqueBaseId;
