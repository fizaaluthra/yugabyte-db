...
add_path(RelOptInfo *parent_rel, Path *new_path)
{
	...
	foreach(p1, parent_rel->pathlist)
	{
		...
		if (remove_old)
		{
			 ...
			 * YB: UniquePath is also generated by build_index_paths
	...
	else
	{
		 ...
		 * YB: UniquePath is also generated by build_index_paths and must
		 * be preserved.
		 */
				  ...
				  List *yb_saop_merge_saop_cols)
{
	...
	/*
	 * For YB relations with no pathkeys, scan direction is irrelevant. PG19
	 * added Assert(indexscandir == Forward || Backward) in create_indexscan_plan,
	 * so we must use ForwardScanDirection rather than NoMovementScanDirection.
	 */
	pathnode->indexscandir = rel->is_yb_relation && pathkeys == NIL ?
		ForwardScanDirection : indexscandir;
		...
		case T_YbBitmapTablePath:
			{
				...
				YbBitmapTablePath *subpath = (YbBitmapTablePath *) path;

				bhpath = makeNode(YbBitmapTablePath);
				memcpy(bhpath, subpath, sizeof(YbBitmapTablePath));
				REPARAMETERIZE_CHILD_PATH(bhpath->bitmapqual);
				new_path = (Path *) bhpath;
			}
			break;
 ...
 *
 * Reuses create_unique_path (PG19 merged UpperUniquePath into UniquePath).
 */
static UniquePath *
 ...
 * - or a UniquePath on top of a distinct index scan path
 */
Path *
							  ...
							  int yb_distinct_nkeys)
{
	...
	if (index->nhashcolumns == 0)
	{
		...
		if (yb_distinct_nkeys >= 0)
			/* pathkeys available. Can use UniquePath here. */
			return (Path *)
