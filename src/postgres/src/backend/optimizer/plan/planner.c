				 ...
				 ParamListInfo boundParams, ExplainState *es)
{
	...
	if (IsYugaByteEnabled())
	{
		...
		if (parse->resultRelation != 0)
		{
			...
			if (targetRte->rtekind == RTE_RELATION && targetRte->perminfoindex > 0)
			{
				RTEPermissionInfo *perminfo = getRTEPermissionInfo(parse->rteperminfos, targetRte);
				 ...
				 */
				perminfo->ybUniqueBaseId = ++(glob->ybBaseRelCnt);
				 ...
				 */
				perminfo->ybHintAlias = hintAlias;
			}
		}
	}
					...
					case T_YbBitmapIndexScan:
						{
							...
							char	   *hintAlias1 = (rte1->perminfoindex > 0)
								? getRTEPermissionInfo(plannedStmt1->permInfos, rte1)->ybHintAlias : NULL;
							char	   *hintAlias2 = (rte2->perminfoindex > 0)
								? getRTEPermissionInfo(plannedStmt2->permInfos, rte2)->ybHintAlias : NULL;

							if (hintAlias1 != NULL && hintAlias2 != NULL)
							{
								plansAreEqual = (strcmp(hintAlias1, hintAlias2) == 0);

								if (trace && !plansAreEqual)
								{
									ereport(INFO,
											(errmsg("\n++ NOT EQUAL : scan1 (%u) hint alias = %s , scan2 (%u) hint alias = %s",
													plan1->ybUniqueId, hintAlias1, plan2->ybUniqueId, hintAlias2)));
								}
							}
							else if (hintAlias1 != NULL && hintAlias2 == NULL)
							{
								if (trace)
								{
									ereport(INFO,
											(errmsg("\n++ NOT EQUAL : scan1 (%u) hint alias = %s , scan2 (%u) hint alias = NULL",
													plan1->ybUniqueId, hintAlias1, plan2->ybUniqueId)));
								}
								...
								if (trace)
								{
									ereport(INFO,
											(errmsg("\n++ NOT EQUAL : scan1 (%u) hint alias = NULL , scan2 (%u) hint alias = %s",
													plan1->ybUniqueId, plan2->ybUniqueId, hintAlias2)));
								}
