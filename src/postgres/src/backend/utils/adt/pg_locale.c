...
pg_newlocale_from_collation(Oid collid)
{
	...
	{
		/*
		 * YB: in the master/tserver DocDB expression-evaluation context
		 * (YbgEvalExpr), init_database_collation() is never called, so
		 * default_locale stays NULL.  Fall back to the C locale, which is
		 * the YB default and prevents a NULL-pointer dereference in callers
		 * such as pg_set_regex_collation.
		 */
