...
typedef struct
{
	...
	List	   *rteperminfos;	/* Optional: for RTEPermissionInfo->ybHintAlias lookup */
...
deparse_context_for_plan_tree(PlannedStmt *pstmt, List *rtable_names)
{
	...
	dpns->rteperminfos = pstmt->permInfos;
 ...
 */
List *
select_rtable_names_for_explain(List *rtable, List *rteperminfos,
								Bitmapset *rels_used)
{
	deparse_namespace dpns;
	...
	memset(&dpns, 0, sizeof(dpns));
	dpns.rtable = rtable;
	dpns.rteperminfos = rteperminfos;
	dpns.subplans = NIL;
	dpns.ctes = NIL;
	dpns.appendrels = NULL;
				 ...
				 Bitmapset *rels_used)
{
	...
	foreach(lc, dpns->rtable)
	{
		...
		if (IsYugaByteEnabled() && dpns->rteperminfos != NULL &&
			rte->perminfoindex > 0 && (!rels_used || bms_is_member(rtindex, rels_used)))
		{
			RTEPermissionInfo *perminfo = getRTEPermissionInfo(dpns->rteperminfos, rte);

			if (perminfo != NULL && perminfo->ybHintAlias != NULL)
				refname = perminfo->ybHintAlias;
		}
					  ...
					  List *parent_namespaces)
{
	...
	dpns->rteperminfos = query->rteperminfos;
	dpns->subplans = NIL;
