...
#define PG_STAT_GET_PROGRESS_COLS	PGSTAT_NUM_PROGRESS_PARAM + 3
	...
	for (curr_backend = 1; curr_backend <= num_backends; curr_backend++)
	{
		 ...
		 */
		if (beentry->st_progress_command != cmdtype && beentry->st_progress_command != PROGRESS_COMMAND_COPY)
			continue;
...
#define YB_BACKEND_XID_COL			PG_STAT_GET_ACTIVITY_COLS
	...
	if (YBIsEnabledInPostgresEnvVar() && yb_enable_pg_locks)
	{
		...
		for (curr_backend = 1; curr_backend <= num_backends; ++curr_backend)
		{
			LocalPgBackendStatus *local_beentry_txn = pgstat_get_local_beentry_by_index(curr_backend);
		const PgBackendStatus *beentry = &local_beentry_txn->backendStatus;
	...
	for (curr_backend = 1; curr_backend <= num_backends; curr_backend++)
	{
			...
			beentry->st_activity_start_timestamp <= yb_txn_rpc_timestamp)
		{
			...
			if (info->is_not_null)
			{
				values[YB_BACKEND_XID_COL] = UUIDPGetDatum((pg_uuid_t *) &info->txn_id);
...
yb_pg_stat_get_backend_catalog_version(PG_FUNCTION_ARGS)
{
	...
	local_beentry = pgstat_get_local_beentry_by_index(beid);
	if (!local_beentry)
		PG_RETURN_NULL();
	beentry = &local_beentry->backendStatus;
