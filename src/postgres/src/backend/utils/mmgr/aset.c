 ...
 * It is never removed from the header or replaced. In PG19, keeper is not a
 * struct member; use KeeperBlock() macro to obtain it.
 */
#define ASET_INITIAL_TOTAL_SIZE(SET) (KeeperBlock(SET)->endptr - ((char *) SET))
...
AllocSetFree(void *pointer)
{
	...
	if (MemoryChunkIsExternal(chunk))
	{
		...
		/* Must be computed before wipe_mem wipes the content */
		size_t		freed_sz = ASET_BLOCK_TOTAL_SIZE(block);
