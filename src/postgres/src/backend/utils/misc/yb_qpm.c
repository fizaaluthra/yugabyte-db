...
qpmProcess(QueryDesc *queryDesc)
{
	...
	{
		/* YB_TODO_PG19MERGE: InstrEndLoop is for per-node NodeInstrumentation;
		 * query-level Instrumentation doesn't loop. Skip. */
		CHECK_GLOBALS;
	...
	if (*hintText != NULL)
	{
		...
		*freePlanText = true;
		...
		if (queryDesc->query_instr != NULL)
		{
			 ...
			 */
			/* YB_TODO_PG19MERGE: total is now instr_time (struct); convert via macro. */
			*totalTime = INSTR_TIME_GET_MILLISEC(queryDesc->query_instr->total);
...
{
	PlanState  *ps;
	/* YB_TODO_PG19MERGE: PG19 PlanState.instrument is NodeInstrumentation *. */
