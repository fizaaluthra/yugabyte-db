...
#include "optimizer/yb_costmodel.h"
...
double yb_transaction_priority = 0.0;
int yb_tcmalloc_sample_period = 1024 * 1024;	/* 1MB */
...
bool check_default_replica_identity(char **newval, void **extra,
									GucSource source);
bool yb_check_neg_catcache_ids(char **newval, void **extra,
							   GucSource source);
void yb_set_neg_catcache_ids(const char *newval, void *extra);
...
bool check_transaction_priority_lower_bound(double *newval, void **extra, GucSource source);
bool check_transaction_priority_upper_bound(double *newval, void **extra, GucSource source);
bool check_yb_explicit_row_locking_batch_size(int *newval, void **extra, GucSource source);
bool yb_check_no_txn(int *newval, void **extra, GucSource source);
bool yb_check_toast_catcache_threshold(int *newval, void **extra, GucSource source);
bool yb_disable_auto_analyze_check_hook(bool *newval, void **extra, GucSource source);
const char *show_tcmalloc_sample_period(void);
static const char *yb_show_maxconnections(void);
void assign_tcmalloc_sample_period(int newval, void *extra);
void assign_yb_pg_batch_detection_mechanism(int new_value, void *extra);
void assign_ysql_upgrade_mode(bool newval, void *extra);
void assign_yb_enable_cbo(int new_value, void *extra);
void assign_yb_enable_optimizer_statistics(bool new_value, void *extra);
void assign_yb_enable_base_scans_cost_model(bool new_value, void *extra);

bool check_yb_disable_pg_snapshot_mgmt_in_repeatable_read(bool *newval, void **extra, GucSource source);
bool check_yb_enable_advisory_locks(bool *newval, void **extra, GucSource source);

void assign_yb_silence_advisory_locks_not_supported_error(bool newval, void *extra);

void assign_yb_enable_pg_stat_statements_rpc_stats(bool newval, void *extra);
...
const struct config_enum_entry yb_cost_model_options[] = {
...
char *yb_effective_transaction_isolation_level_string;
char *yb_xcluster_consistency_level_string;
char *yb_read_time_string;
char *yb_neg_catcache_ids_string;
 ...
 */
static const char *const pg_attribute_unused() YbDbAdminVariables[] = {
	...
	set_stack_value(struct config_generic *gconf, config_var_value *val)
{
	switch (gconf->vartype)
	{
		...
		case PGC_INT:
			...
			break;
		case PGC_OID:	/* YB: OID GUC type */
			val->val.oidval = *((struct yb_config_oid *) gconf)->variable;
			break;
		case PGC_REAL:
...
#ifdef USE_ASSERT_CHECKING
		/* YB: Temporarily relaxed for PG19 merge - some YB GUCs may not pass */
		if (!check_GUC_init(hentry->gucvar))
			elog(LOG, "YB: check_GUC_init failed for %s (non-fatal during PG19 merge)",
				 hentry->gucvar->name);
#endif
...
/*
 * Try to parse value as an OID (unsigned 32-bit integer).
 *
 * If the string parses okay, return true, else false.
 * If okay and result is not NULL, return the value in *result.
 * If not okay and hintmsg is not NULL, *hintmsg is set to a suitable
...
parse_oid(const char *value, Oid *result, const char **hintmsg)
{
	...
	/* allow whitespace after the number */
	while (isspace((unsigned char) *endptr))
		endptr++;
	if (*endptr != '\0')
		return false;

	if (errno == ERANGE || val > PG_UINT32_MAX)
	{
		if (hintmsg)
			*hintmsg = gettext_noop("Value exceeds OID range.");
		return false;
	}
		...
		case PGC_OID:	/* YB: OID GUC type */
			{
				...
				if (newval->oidval < conf->min || newval->oidval > conf->max)
				{
					ereport(elevel,
							(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
							 errmsg("%u is outside the valid range for parameter \"%s\" (%u .. %u)",
...
GetConfigOptionResetString(const char *name)
{
	...
	switch (record->vartype)
	{
		...
		case PGC_OID:	/* YB: OID GUC type */
			{
				struct yb_config_oid *lconf = (struct yb_config_oid *) record;
...
estimate_variable_size(struct config_generic *gconf)
{
	...
	switch (gconf->vartype)
	{
		...
		case PGC_OID:	/* YB: OID GUC type */
			{
				/* Oid max value is 4294967295, i.e. 10 chars */
				valsize = 10 + 1;
			}
			break;
 ...
 */
static const char * pg_attribute_unused()
yb_show_maxconnections(void)
{
	...
	{
		yb_adj_max_con -= (SuperuserReservedConnections + ReservedConnections + max_wal_senders);
	}
