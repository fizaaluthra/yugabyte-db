...
YbPreloadCatalogCache(int cache_id, int idx_cache_id)
{
	...
	if (yb_debug_log_catcache_events)
	{
		...
		elog(LOG, "YbPreloadCatalogCache: " INT64_FORMAT " entries added for "
			 "cache id %d, index oid %u (relation %s), took " INT64_FORMAT " us",
			 (int64) scanned, cache->id, cache->cc_indexoid, cache->cc_relname,
			 (int64) INSTR_TIME_GET_MICROSEC(duration));
	}
...
YbCheckCatalogCacheIds()
{
	...
	YB_CHECK_CATALOG_CACHE_ID(PUBLICATIONNAME, 48);
	YB_CHECK_CATALOG_CACHE_ID(PUBLICATIONNAMESPACE, 49);
	YB_CHECK_CATALOG_CACHE_ID(PUBLICATIONNAMESPACEMAP, 50);
	YB_CHECK_CATALOG_CACHE_ID(PUBLICATIONOID, 51);
	YB_CHECK_CATALOG_CACHE_ID(PUBLICATIONREL, 52);
	YB_CHECK_CATALOG_CACHE_ID(PUBLICATIONRELMAP, 53);
	YB_CHECK_CATALOG_CACHE_ID(RANGEMULTIRANGE, 54);
	YB_CHECK_CATALOG_CACHE_ID(RANGETYPE, 55);
	YB_CHECK_CATALOG_CACHE_ID(RELNAMENSP, 56);
	YB_CHECK_CATALOG_CACHE_ID(RELOID, 57);
	YB_CHECK_CATALOG_CACHE_ID(REPLORIGIDENT, 58);
	YB_CHECK_CATALOG_CACHE_ID(REPLORIGNAME, 59);
	YB_CHECK_CATALOG_CACHE_ID(RULERELNAME, 60);
	YB_CHECK_CATALOG_CACHE_ID(SEQRELID, 61);
	YB_CHECK_CATALOG_CACHE_ID(STATEXTDATASTXOID, 62);
	YB_CHECK_CATALOG_CACHE_ID(STATEXTNAMENSP, 63);
	YB_CHECK_CATALOG_CACHE_ID(STATEXTOID, 64);
	YB_CHECK_CATALOG_CACHE_ID(STATRELATTINH, 65);
	YB_CHECK_CATALOG_CACHE_ID(SUBSCRIPTIONNAME, 66);
	YB_CHECK_CATALOG_CACHE_ID(SUBSCRIPTIONOID, 67);
	YB_CHECK_CATALOG_CACHE_ID(SUBSCRIPTIONRELMAP, 68);
	YB_CHECK_CATALOG_CACHE_ID(TABLESPACEOID, 69);
	YB_CHECK_CATALOG_CACHE_ID(TRFOID, 70);
	YB_CHECK_CATALOG_CACHE_ID(TRFTYPELANG, 71);
	YB_CHECK_CATALOG_CACHE_ID(TSCONFIGMAP, 72);
	YB_CHECK_CATALOG_CACHE_ID(TSCONFIGNAMENSP, 73);
	YB_CHECK_CATALOG_CACHE_ID(TSCONFIGOID, 74);
	YB_CHECK_CATALOG_CACHE_ID(TSDICTNAMENSP, 75);
	YB_CHECK_CATALOG_CACHE_ID(TSDICTOID, 76);
	YB_CHECK_CATALOG_CACHE_ID(TSPARSERNAMENSP, 77);
	YB_CHECK_CATALOG_CACHE_ID(TSPARSEROID, 78);
	YB_CHECK_CATALOG_CACHE_ID(TSTEMPLATENAMENSP, 79);
	YB_CHECK_CATALOG_CACHE_ID(TSTEMPLATEOID, 80);
	YB_CHECK_CATALOG_CACHE_ID(TYPENAMENSP, 81);
	YB_CHECK_CATALOG_CACHE_ID(TYPEOID, 82);
	YB_CHECK_CATALOG_CACHE_ID(USERMAPPINGOID, 83);
	YB_CHECK_CATALOG_CACHE_ID(USERMAPPINGUSERSERVER, 84);
	YB_CHECK_CATALOG_CACHE_ID(YBCONSTRAINTRELIDTYPIDNAME, 85);
	YB_CHECK_CATALOG_CACHE_ID(YBTABLEGROUPOID, 86);
	 ...
	 */
	static_assert(SysCacheSize == 87, "new catalog cache id added");
}
