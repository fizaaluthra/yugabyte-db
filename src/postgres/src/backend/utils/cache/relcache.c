		...
		switch (relation->rd_rel->relpersistence)
		{
			...
			case RELPERSISTENCE_TEMP:
				...
				else
				{
					 ...
					 * we set rd_backend to the invalid proc number.
					 */
					relation->rd_backend = INVALID_PROC_NUMBER;
					relation->rd_islocaltemp = false;
				}
...
YbApplyAttr(YbAttrProcessorState *state, Relation attrel, HeapTuple htup)
{
	...
	/* YB: PG19 requires CompactAttribute to be populated after Form_pg_attribute changes */
...
YbCompleteAttrProcessingImpl(const YbAttrProcessorState *state)
{
	...
	/*
	 * attcacheoff was removed from FormData_pg_attribute in PG19. The value
	 * for the first attribute was zero. This YB code is no longer needed.
	 */
...
retry:
	...
	if (yb_debug_log_catcache_events && insertIt)
	{
		...
		elog(LOG, "Building relcache entry %p for %s (oid %u) took %lld us", (void *) relation,
			 RelationGetRelationName(relation), RelationGetRelid(relation),
			 (long long) INSTR_TIME_GET_MICROSEC(duration));
	}
...
#define NUM_CRITICAL_LOCAL_RELS 4	/* fix if you change list above */
	...
	if (IsYugaByteEnabled())
	{
		 ...
		 */
		/* When very short on connections, skip the extra relcache init optimization. */
		if (MaxConnections <= 3)
