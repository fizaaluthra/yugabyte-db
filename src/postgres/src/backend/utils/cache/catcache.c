				...
				List *current_list)
{
	 ...
	 * Initialize lbucket if this is the first list search for this cache.
	 */
	...
	{
		Index		lHashIdx = HASH_INDEX(lHashValue, cache->cc_nlbuckets);

		dlist_push_head(&cache->cc_lbucket[lHashIdx], &cl->cache_elem);
	}
	cache->cc_nlist++;
}
...
void
YbSetAdditionalNegCacheIds(struct List *neg_cache_ids)
{
...
yb_log_catcache_stats(PG_FUNCTION_ARGS)
{
	 ...
	 *
	 * If the given process is a backend, use its proc number in
