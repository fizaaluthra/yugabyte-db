...
/* YB: Helper functions for pg_inherits cache reference resource management. */
static void ResOwnerReleaseYbPgInheritsRef(Datum res);
static char *ResOwnerPrintYbPgInheritsRef(Datum res);

/* YB: Resource owner descriptor for tracking pg_inherits cache references. */
static const ResourceOwnerDesc ybpginherits_resowner_desc =
{
	.name = "YB pg_inherits cache reference",
							 ...
							 bool isTopLevel)
{
	...
	if (phase == RESOURCE_RELEASE_BEFORE_LOCKS)
	{
		...
		/*
		 * YB pg_inherits cache references are now tracked through the generic
		 * ResourceOwnerDesc mechanism and released by ResourceOwnerReleaseAll
		 * above.
		 */
	}
...
/*
 * YB: Resource owner support for pg_inherits cache references.
 *
 * In PG19, resource types are tracked via the generic ResourceOwnerDesc
 * mechanism rather than per-type ResourceArrays.
 */

/* YB: Release a pg_inherits cache entry when resource owner is released. */
static void
ResOwnerReleaseYbPgInheritsRef(Datum res)
{
	ReleaseYbPgInheritsCacheEntry((YbPgInheritsCacheEntry) DatumGetPointer(res));
}

/* YB: Print debug information for a pg_inherits cache entry. */
static char *
ResOwnerPrintYbPgInheritsRef(Datum res)
{
	YbPgInheritsCacheEntry entry = (YbPgInheritsCacheEntry) DatumGetPointer(res);

	return psprintf("YB pg_inherits cache entry for OID %u", entry->oid);
}

/* YB: Ensure resource owner has space for pg_inherits cache references. */
void
...
/* YB: Remember a pg_inherits cache entry in the resource owner. */
void
									 ...
									 YbPgInheritsCacheEntry entry)
{
	ResourceOwnerRemember(owner, PointerGetDatum(entry),
						  &ybpginherits_resowner_desc);
}

/* YB: Forget a pg_inherits cache entry in the resource owner. */
void
