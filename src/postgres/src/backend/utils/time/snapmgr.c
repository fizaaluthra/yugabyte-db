...
{
	/* YB: Skip when YB is not enabled (e.g., standalone initdb/bootstrap) */
...
YbLogSnapshotData(const char *msg, const SnapshotData *snap, bool log_stack_trace)
{
	...
	bool		yb_is_catalog_snapshot = (CatalogSnapshot != NULL && snap == CatalogSnapshot);

	elog(YbSnapshotMgmtLogLevel(),
		 "%s %s read point: %" PRIu64 ", effective isolation level: %d. %s",
		 msg,
		 yb_is_catalog_snapshot ? "(catalog)" : "(regular)",
