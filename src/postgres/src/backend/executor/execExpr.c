				...
				Datum *resv, bool *resnull)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_YbExprColrefDesc:
			{
				/*
				 * YbExprColrefDesc is a DocDB column reference descriptor used
				 * in the YB single-row modify optimization. When encountered
				 * in the standard expression evaluator (e.g., inside SQL
				 * functions via SPI), treat it as a simple Var-like slot
				 * access using its attno.
				 */
				YbExprColrefDesc *colref = (YbExprColrefDesc *) node;

				scratch.opcode = EEOP_INNER_VAR;
				scratch.d.var.attnum = colref->attno - 1;
				scratch.d.var.vartype = colref->typid;
				scratch.resvalue = resv;
				scratch.resnull = resnull;
				ExprEvalPushStep(state, &scratch);
				break;
