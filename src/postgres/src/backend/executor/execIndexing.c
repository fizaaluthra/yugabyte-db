								...
								EState *estate)
{
	...
	if (indnkeyatts == 1)
	{
		FormData_pg_attribute att = *TupleDescAttr(index->rd_att, 0);
	...
	index_scan = index_beginscan(heap, index, estate->es_snapshot, NULL, 1, 0);
								  ...
								  YbInsertOnConflictBatchState *yb_ioc_state)
{
	...
	IndexScanDesc index_only_scan = index_beginscan(heap, index, estate->es_snapshot, NULL, nkeys, 0);
	ItemPointer tid;
