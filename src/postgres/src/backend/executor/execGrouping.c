...
/* YB: Prepare equality expression for outer tuple expressions in BNL (Block Nested Loop) joins. */
ExprState *
...
/* YB: Extended tuple hash table builder supporting expression-based keys for BNL joins. */
TupleHashTable
						 ...
						 bool use_variable_hash_iv)
{
	...
	hashtable = (TupleHashTable) palloc0(sizeof(TupleHashTableData));

	hashtable->numCols = numCols;
	hashtable->yb_keyColExprs = keyColExprs;
	hashtable->keyColIdx = NULL;
	hashtable->yb_tab_hash_funcs = hashfunctions;
	hashtable->tab_collations = NULL;
	hashtable->tuplescxt = tablecxt;
	hashtable->tempcxt = tempcxt;
	hashtable->additionalsize = additionalsize;
	hashtable->tableslot = NULL;	/* will be made on first lookup */
	hashtable->inputslot = NULL;
	hashtable->yb_in_hash_funcs = NULL;
	...
	if (use_variable_hash_iv)
		hashtable->yb_hash_iv = murmurhash32(ParallelWorkerNumber);
	else
		hashtable->yb_hash_iv = 0;
					 ...
					 bool *isnew, uint32 *hash)
{
	...
	hashtable->yb_in_hash_funcs = hashtable->yb_tab_hash_funcs;
...
/*
 * YB: FindTupleHashEntry variant using FmgrInfo hash functions and
 * keyColIdx instead of ExprState hash expressions.
 */
TupleHashEntry
YbFindTupleHashEntry(TupleHashTable hashtable, TupleTableSlot *slot,
					 ExprState *eqcomp,
					 FmgrInfo *hashfunctions,
					 AttrNumber *keyColIdx)
{
	...
	hashtable->inputslot = slot;
	hashtable->in_hash_expr = NULL;
	hashtable->yb_in_hash_funcs = hashfunctions;
							...
							MinimalTuple tuple)
{
	...
	uint32		hashkey = hashtable->yb_hash_iv;
	TupleTableSlot *slot;
	bool		isnull;
	FmgrInfo   *hashfunctions;
	int			i;
	...
	if (tuple == NULL)
	{
		...
		else
		{
			...
			hashfunctions = hashtable->yb_in_hash_funcs;
	...
	else
	{
		...
		else
		{
			...
			ExecStoreMinimalTuple(tuple, slot, false);
			hashfunctions = hashtable->yb_tab_hash_funcs;
		}
	}
