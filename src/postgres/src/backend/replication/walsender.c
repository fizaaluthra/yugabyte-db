	...
	else
	{
		...
		if (!IsYugaByteEnabled())
			ReplicationSlotCreate(cmd->slotname, true,
								  ...
								  cmd->plugin, CRS_NOEXPORT_SNAPSHOT, NULL,
		...
		if (snapshot_action == CRS_EXPORT_SNAPSHOT)
		{
			...
			need_full_snapshot = true;
				 ...
				 (!IsYugaByteEnabled() || *YBCGetGFlags()->ysql_enable_pg_export_snapshot))
		{
			if (IsYugaByteEnabled())
			{
				if (XactDeferrable)
		...
		if (IsYugaByteEnabled())
		{
			...
			ReplicationSlotCreate(cmd->slotname, true, RS_PERSISTENT, two_phase,
								  failover, false, cmd->plugin, snapshot_action,
...
XLogSendLogical(void)
{
		...
		logical_decoding_ctx->reader->EndRecPtr >= flushPtr)
	{
		...
		if (am_cascading_walsender)
			...
			flushPtr = IsYugaByteEnabled() ? YBCGetFlushRecPtr() : GetFlushRecPtr(NULL);
	}
