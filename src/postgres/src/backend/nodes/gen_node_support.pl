...
" unless $struct_no_equal;
	...
	foreach my $f (@{ $node_type_info{$n}->{fields} })
	{
		...
		# YB: handle unknown embedded struct types (e.g., YbPushdownExprs,
		# YbPlanInfo) by treating them as inline structs (direct assignment
		# for copy, memcmp for equal).
		# This is a pragmatic workaround for YB-specific non-Node struct
		# types that are embedded by value in Node structs.
		elsif ($t !~ /\*/)
		{
			print $cff "\tnewnode->$f = from->$f;\n" unless $copy_ignore;
			print $eff "\tif (memcmp(&newnode->$f, &a->$f, sizeof(newnode->$f)) != 0)\n\t\treturn false;\n" unless $equal_ignore;
		}
		# YB: for unknown pointer types, allow if both copy and equal are
		# already handled via field-level attributes (copy_as_scalar, etc.)
		else
		{
			die
...
";
	...
	foreach my $f (@{ $node_type_info{$n}->{fields} })
	{
		...
		if ($read_write_ignore)
		{
			...
			# YB: allow read_write_ignore without read_as() -- just skip
			# the field entirely (not serialized, stays zero-initialized).
			next;
		}
...
! unless $no_read;
		...
		# YB: skip unknown embedded struct types for write/read.
		# These are YB-specific non-Node struct types (e.g., YbPushdownExprs,
		# YbPlanInfo) embedded by value.  They are not serialized.
		elsif ($t !~ /\*/)
		{
			# silently skip -- not serialized
		}
		else
		{
			die
