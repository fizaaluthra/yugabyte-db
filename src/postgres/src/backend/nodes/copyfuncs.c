...
/*
 * YB: Custom copy for YbUpdateAffectedEntities.
 * This struct has inline sub-structs that gen_node_support.pl cannot parse,
 * so we provide a handwritten deep copy.
 */
...
_copyYbUpdateAffectedEntities(const YbUpdateAffectedEntities *from)
{
	...
	int			nentities = YB_UPDATE_AFFECTED_ENTITIES_NUM_ENTITIES(from);
	int			nfields = YB_UPDATE_AFFECTED_ENTITIES_NUM_FIELDS(from);

	if (nentities > 0)
	{
		newnode->entity_list = palloc(nentities * sizeof(struct YbUpdateEntity));
		memcpy(newnode->entity_list, from->entity_list,
			   nentities * sizeof(struct YbUpdateEntity));
	}

	if (nfields > 0)
	{
		newnode->col_info_list = palloc(nfields * sizeof(struct YbUpdateColInfo));
		for (int i = 0; i < nfields; i++)
		{
			newnode->col_info_list[i].attnum = from->col_info_list[i].attnum;
			newnode->col_info_list[i].entity_refs =
				list_copy(from->col_info_list[i].entity_refs);
		}
	}
