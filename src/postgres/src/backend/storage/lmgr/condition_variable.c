...
YbConditionVariableCancelSleepForProc(volatile PGPROC *proc)
{
	...
	SpinLockAcquire(&cv->mutex);
	if (proclist_contains(&cv->wakeup, GetNumberFromPGProc((PGPROC *) proc), cvWaitLink))
		proclist_delete(&cv->wakeup, GetNumberFromPGProc((PGPROC *) proc), cvWaitLink);
	SpinLockRelease(&cv->mutex);
	cv_sleep_target = NULL;
}
...
{
	int			pgprocno = GetNumberFromPGProc((PGPROC *) given_proc);
	PGPROC	   *proc = NULL;
