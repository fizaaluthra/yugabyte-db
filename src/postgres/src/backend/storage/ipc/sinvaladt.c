...
/*
 * YB: Forward declaration for internal helper that supports both self and
 * arbitrary-proc cleanup (used by CleanupInvalidationStateForProc).
 */
static void CleanupInvalidationStateInternal(SISeg *segP, PGPROC *proc, bool update_lxid);

/*
 * CleanupInvalidationState
 *		Mark the current backend as no longer active.
 *
 * YB: Delegates to CleanupInvalidationStateInternal with update_lxid=true.
 */
void
CleanupInvalidationState(int status, Datum arg)
{
	...
	CleanupInvalidationStateInternal(segP, MyProc, true);
}

/*
 * YB: Cleanup invalidation state for an arbitrary PGPROC (not necessarily
 * the current backend).  Used during postmaster crash cleanup.
 */
void
CleanupInvalidationStateForProc(PGPROC *proc)
{
	CleanupInvalidationStateInternal(shmInvalBuffer, proc, false);
}
 ...
 *
 * YB: Accepts a proc argument and update_lxid flag so it can be reused
 * for both self-cleanup and arbitrary-proc cleanup.
 */
static void
CleanupInvalidationStateInternal(SISeg *segP, PGPROC *proc, bool update_lxid)
{
	ProcState  *stateP;
	int			i;
	...
	stateP = &segP->procState[GetNumberFromPGProc(proc)];

	/*
	 * Update next local transaction ID for next holder of this proc number.
	 * YB: Skip when cleaning up an arbitrary proc (update_lxid == false)
	 * since nextLocalTransactionId is per-backend.
	 */
	if (update_lxid)
	...
	{
		if (segP->pgprocnos[i] == GetNumberFromPGProc(proc))
		{
