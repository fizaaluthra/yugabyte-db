					   ...
					   Node *object, Relation relation)
{
	switch (objtype)
	{
		...
		case OBJECT_TRIGGER:
			...
			break;
		case OBJECT_DATABASE:
			if (!object_ownercheck(DatabaseRelationId, address.objectId, roleid) &&
				!IsYbDbAdminUser(roleid))	/* YB */
				aclcheck_error(ACLCHECK_NOT_OWNER, objtype,
							   strVal(object));
			break;
		case OBJECT_TYPE:
		...
		case OBJECT_SUBSCRIPTION:
		case OBJECT_TABLESPACE:
			if (!object_ownercheck(address.classId, address.objectId, roleid) && !IsYbDbAdminUser(roleid))
				aclcheck_error(ACLCHECK_NOT_OWNER, objtype,
							   strVal(object));
			break;
		case OBJECT_COLLATION:
