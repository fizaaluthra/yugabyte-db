...
static void AppendAttributeTuples(Relation indexRelation, const Datum *attopts, const NullableDatum *stattargets, bool yb_relisshared);
					  ...
					  bool yb_relisshared)
{
	...
	InsertPgAttributeTuples(pg_attribute, indexTupDesc, InvalidOid, attrs_extra, indstate, yb_relisshared);
				   ...
				   double reltuples)
{
	...
	if (update_stats)
	{
		...
		if (rel->rd_rel->relkind != RELKIND_INDEX &&
			!(IsYugaByteEnabled() && IsYBRelation(rel)))
	...
	if (dirty)
	{
		systable_inplace_update_finish(state, tuple, false);
				 ...
				 List *changedIndexSplitOpts)
{
	...
	if ((flags & REINDEX_REL_PROCESS_TOAST) && OidIsValid(toast_relid))
	{
		...
		result |= reindex_relation(stmt, toast_relid, flags, &newparams,
								   is_yb_table_rewrite, yb_copy_split_options,
								   NIL, NIL);
	}
