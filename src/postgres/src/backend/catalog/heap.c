			...
			bool create_storage)
{
	...
	*relminmxid = InvalidMultiXactId;
	...
	else if (IsYugaByteEnabled() && shared_relation)
		/*
		 * YB note:
		 * Shared relations have no relfilenumber in YB mode.
		 * Note that relations defined with BKI_BOOTSTRAP option have no relfilenumber
		 * either, but YB tables shouldn't use it.
		 * Guard with IsYugaByteEnabled() so standalone initdb follows upstream PG path.
		 */
...
YbSetInitdbPermissions(Oid relid, char relkind, bool relisshared)
{
	...
	if (IsYugaByteEnabled())
		...
		CatalogTupleInsert(pg_init_privs, pg_init_privs_tuple);
...
RemoveAttributeById(Oid relid, AttrNumber attnum)
{
	...
	/*
	 * YB: save a copy of the unmodified tuple before we rename the column.
	 * CatalogTupleDelete needs the original attname so that
	 * CatalogIndexDelete can locate and remove the correct secondary-index
	 * entry in the pg_attribute_relid_attnam_index.
	 */
	HeapTuple	yb_old_tuple = NULL;
	if (IsYugaByteEnabled())
		yb_old_tuple = heap_copytuple(tuple);
	...
	{
		/*
		 * Delete with the original tuple so CatalogIndexDelete removes
		 * the old secondary-index entry (e.g. the ATTNAME entry for the
		 * original column name), then insert the modified tuple.
		 */
		CatalogTupleDelete(attr_rel, yb_old_tuple);
		CatalogTupleInsert(attr_rel, tuple);
		heap_freetuple(yb_old_tuple);
	}
	else
	{
						  ...
						  const char *queryString)
{
	...
	foreach_node(Constraint, cdef, newConstraints)
	{
		...
		/* YB added: shared relations can not have CHECK constraints in YB mode */
		if (IsYugaByteEnabled() && rel->rd_rel->relisshared &&
			!IsBootstrapProcessingMode() && !YBCIsInitDbModeEnvVarSet())
...
CopyStatistics(Oid fromrelid, Oid torelid)
{
	...
	while (HeapTupleIsValid((tup = systable_getnext(scan))))
	{
		...
		CatalogTupleInsertWithInfo(statrel, tup, indstate, false);
