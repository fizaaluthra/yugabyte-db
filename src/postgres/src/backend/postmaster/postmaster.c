...
ServerLoop(void)
{
	...
	/* YB: Set OOM score adjustment for backend processes on Linux */
#ifdef __linux__
...
process_pm_child_exit(void)
{
	...
	while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0)
	{
		...
		for (i = 0; i < ProcGlobal->allProcCount; i++)
		{
			...
			/*
			 * YB: YB_TODO_PG19_MERGE: isBackgroundWorker was removed from PGPROC in PG19.
			 * Need to find an alternative way to detect bgworkers for crash handling.
			 */
...
CleanupKilledProcess(PGPROC *proc)
{
	...
	/* From ProcKill */
	ReplicationSlotCleanupForProc(proc, false);
