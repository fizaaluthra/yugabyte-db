...
static ObjectAddress ATExecAddIndex(List **yb_wqueue, AlteredTableInfo *tab,
...
static void GetForeignKeyCheckTriggers(Relation trigrel,
									   ...
									   Oid *updateTriggerOid);
static void ATExecDropConstraint(List **yb_wqueue, AlteredTableInfo *tab,
								 Relation *yb_mutable_rel, const char *constrName,
...
static ObjectAddress dropconstraint_internal(Relation rel,
											 ...
											 bool recurse, bool recursing,
											 bool missing_ok, LOCKMODE lockmode,
											 List **yb_wqueue, AlteredTableInfo *yb_tab);
...
ExecuteTruncate(TruncateStmt *stmt, bool yb_is_top_level, List **yb_relids)
{
	...
	ExecuteTruncateGuts(rels, relids, relids_logged,
						stmt->behavior, stmt->restart_seqs, false, yb_is_top_level);
				  ...
				  List **yb_rollback_handles)
{
	...
	for (AlterTablePass pass = 0; pass < AT_NUM_PASSES; pass++)
	{
		...
		foreach(ltab, *wqueue)
		{
			 ...
			 *
			 * For alter type, do not perform this work if it is a YB relation
			 * because we will have created an entirely new table, so there is
			 * no need for cleanup.
			 */
		  ...
		  AlterTableUtilityContext *context)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_ValidateConstraint: /* VALIDATE CONSTRAINT */
			...
			break;
		case AT_DropConstraint: /* DROP CONSTRAINT */
			ATExecDropConstraint(wqueue, tab, yb_mutable_rel, cmd->name,
								 cmd->behavior, cmd->recurse,
								 cmd->missing_ok, lockmode);
			break;
		case AT_AlterColumnType:	/* ALTER COLUMN TYPE */
	...
	foreach(ltab, *wqueue)
	{
		...
		if (tab->rewrite > 0 && tab->relkind != RELKIND_SEQUENCE)
		{
			...
			if (IsYugaByteEnabled() && tab->relkind == RELKIND_PARTITIONED_TABLE)
			{
				...
				reindex_relation(NULL, RelationGetRelid(OldHeap), 0, &reindex_params,
								 true /* is_yb_table_rewrite */ ,
								 !tab->yb_skip_copy_split_options,
					 ...
					 bool missing_ok, LOCKMODE lockmode)
{
	...
	if (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		dropconstraint_internal(*yb_mutable_rel, tuple, behavior, recurse, false,
							missing_ok, lockmode, yb_wqueue, tab);
		found = true;
	}
						...
						List **yb_wqueue, AlteredTableInfo *yb_tab)
{
	...
	if (IsYBRelation(rel) && con->contype == CONSTRAINT_PRIMARY)
	{
		if (!yb_enable_alter_table_rewrite)
		{
			/* Table will be re-created without a dummy PK index. */
			rel = YbATCloneRelationSetPrimaryKey(rel,
												 NULL /* stmt */ ,
												 NULL /* result */ );
			 ...
			 */
			if (yb_tab)
				yb_tab->relid = RelationGetRelid(rel);
		}
		else
		{
			/*
...
AttachPartitionEnsureIndexes(List **wqueue, Relation rel, Relation attachrel)
{
	...
	foreach(cell, idxes)
	{
		...
		if (!found)
		{
			...
			DefineIndex(NULL, RelationGetRelid(attachrel), stmt, InvalidOid,
						RelationGetRelid(idxRel),
						InvalidOid,
						-1,
			  ...
			  const AttrMap *attmap, int altered_old_attnum)
{
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		stmt->defnames = stringToQualifiedNameList(orig_stats_name, NULL);
		CreateStatistics(stmt, false);
	}
	systable_endscan(scan);
...
YbATCopyMiscMetadata(Relation old_rel, Relation new_rel, const AttrMap *attmap)
{
	...
	for (int attno = 1; attno <= RelationGetDescr(new_rel)->natts; attno++)
	{
		...
		/*
		 * YB: attstattarget was removed from pg_attribute in PG19.
		 * Statistics targets are now managed via pg_attribute_ext.
		 */
							...
							const AttrMap *confkey_attmap)
{
	...
	Datum		conkey_val = SysCacheGetAttrNotNull(CONSTROID, tuple,
												   Anum_pg_constraint_conkey);

	/* attnums of referenced columns. */
	Datum		confkey_val = SysCacheGetAttrNotNull(CONSTROID, tuple,
													Anum_pg_constraint_confkey);

	/* equality operators for PK = FK comparisons */
	Datum		pfeqop_val = SysCacheGetAttrNotNull(CONSTROID, tuple,
													Anum_pg_constraint_conpfeqop);

	/* equality operators for PK = PK comparisons */
	Datum		ppeqop_val = SysCacheGetAttrNotNull(CONSTROID, tuple,
													Anum_pg_constraint_conppeqop);

	/* equality operators for FK = FK comparisons */
	Datum		ffeqop_val = SysCacheGetAttrNotNull(CONSTROID, tuple,
	...
	/* Look for an index matching the column list */
	index_oid = transformFkeyCheckAttrs(fk_rel, numkeys, confkey, false, index_opclasses, NULL);
...
{
	Datum		conkey_val = SysCacheGetAttrNotNull(CONSTROID, constraint_tuple,
												   Anum_pg_constraint_conkey);
	int			numkeys = ARR_DIMS(DatumGetArrayTypeP(conkey_val))[0];
	int16		conkey[numkeys];
	...
	Datum		confkey_val = SysCacheGetAttrNotNull(CONSTROID, constraint_tuple,
													Anum_pg_constraint_confkey);
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		switch (con_form->contype)
		{
			case CONSTRAINT_CHECK:
				{
					...
					else
					{
						Node	   *expr;
						bool		found_whole_row;
						Datum		conbin_val = SysCacheGetAttrNotNull(CONSTROID, tuple,
																			Anum_pg_constraint_conbin);
			...
			case CONSTRAINT_NOTNULL:
				/* NOT NULL constraints are carried over via column definitions. */
				break;
