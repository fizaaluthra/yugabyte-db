			 ...
			 ParamListInfo params, DestReceiver *dest)
{
	...
	/*
	 * YB: When yb_explain_hide_non_deterministic_fields is set, force timing
	 * off so that EXPLAIN output is deterministic for regression tests.
	 * In PG15 this was done inline via YbIsTimingNeeded(); PG19 moved option
	 * parsing to ParseExplainOptionList() so we apply the override here.
	 */
	if (yb_explain_hide_non_deterministic_fields)
		es->timing = false;
