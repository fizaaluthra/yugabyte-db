...
AlterSequence(ParseState *pstate, AlterSeqStmt *stmt)
{
	...
	/*
	 * YB: In YB mode, sequence data lives in DocDB, not in a buffer page.
	 * The buffer page is never initialized with SEQ_MAGIC, so
	 * read_seq_tuple would fail. Guard the upstream read with
	 * !IsYugaByteEnabled() and read from DocDB instead.
	 */
	if (!IsYugaByteEnabled())
	{
			...
			List **owned_by)
{
	...
	*owned_by = NIL;
	...
	int64		ysql_seq_cache_minval = (int64) *YBCGetGFlags()->ysql_sequence_cache_minval;
